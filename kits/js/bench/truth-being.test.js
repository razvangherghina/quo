...
class Dog {
  ...
  async vaccinated() {
    const record = this._quo.relation('clinic');
...
class Walker {
  ...
  async walk(minutes) {
    const rex = this._quo.relation('rex');
    if (!rex) return false;
...
test('1. Alice lets Bob walk Rex: a grant, an accept, and Walker holds a handle', async () => {
  ...
  await walker._quo.accept(invitation, { label: 'rex' });
  assert.equal(await walker._quo.relation('rex').name(), 'Rex');
...
test('2. Bob narrows what Alice sees: Profile is granted, Walker never is', async () => {
  ...
  const [handle] = await rex._quo.accept(await profile._quo.grant(profile), { label: 'bob' });
  ...
  assert.equal(walker._quo.standings().length, 0);
});
...
test('3. the chain: Bob asks Rex, Rex asks Record, and the clinic sees Rex, not Bob', async () => {
  ...
  record.vaccinated = function vaccinated() {
    callers.push(this._quo.caller.voice);
    return true;
  };
  await rex._quo.accept(await record._quo.grant(record), { label: 'clinic' });
  await walker._quo.accept(await rex.invite(), { label: 'rex' });
  assert.equal(await walker._quo.relation('rex').vaccinated(), true);
  // Rex's voice at the clinic is the one the clinic minted for Rex; Bob has
  // no standing there and could not ask directly.
  assert.equal(callers.length, 1);
  assert.deepEqual(callers[0], record._quo.standings()[0].voice);
});
...
test('3b. the leash shrinks by one hop along the chain, and a being never widens it', async () => {
  ...
  rex.vaccinated = async function vaccinated() {
    leashes.push(this._quo.leash.hops);
    return (await this._quo.relation('clinic').vaccinated()) ?? null;
  };
  record.vaccinated = function vaccinated() {
    leashes.push(this._quo.leash.hops);
    return true;
  ...
  await rex._quo.accept(await record._quo.grant(record), { label: 'clinic' });
  await walker._quo.accept(await rex.invite(), { label: 'rex' });
  await walker._quo.relation('rex').vaccinated();
...
test('4. subscription is a grant backwards: Inbox is the callback, and a push is an ask', async () => {
  ...
  const [bob] = await rex._quo.accept(await walker._quo.grant(walker), { label: 'walker' });
...
test('5. Alice fires Bob: amend, and the next call is silence', async () => {
  const { rex, walker } = await world();
  await walker._quo.accept(await rex.invite(), { label: 'rex' });
  const handle = walker._quo.relation('rex');
  assert.equal(await handle.logWalk(5n), true);
  const [bob] = rex._quo.standings();
  rex._quo.amend(bob.voice, { remove: [rex] });
...
test('silence after a write: resending the identical envelope is honoured at most once', async () => {
  ...
  const sealed = await handle._quo.seal('logWalk', 40n);
  assert.equal(await handle._quo.send(sealed), true);
  assert.equal(await handle._quo.send(sealed), null);
...
test('a same-warden call goes through the handle: asynchronous, no seal, one shape', async () => {
  ...
  const { handle } = await rex._quo.hold(new Dog('Pup'), { blueprint: DOG, label: 'pup' });
  const reasons = [];
  ...
  assert.equal(await rex._quo.relation('pup').logWalk(3n), true);
...
test('a migration carries one being: what Rex minted stays where it was minted', async () => {
  ...
  const { being: landingPk } = await rex._quo.hold(landing, { blueprint: INBOX, label: 'landing' });
  await walker._quo.accept(await phone.grant(landingPk), { label: 'landing' });
  assert.equal(walker._quo.relation('landing') !== null, true);

  const cargo = pack(phone, rex._quo.being);
...
test('accept answers one handle per being the standing names', async () => {
  ...
  const [held] = rex._quo.standings();
  rex._quo.amend(held.voice, { add: [inbox] });

  const handles = await walker._quo.accept(invitation);
  assert.equal(handles.length, 2);
  const atRex = handles.find((one) => hex(one._quo.being) === hex(rex._quo.being));
  const atInbox = handles.find((one) => hex(one._quo.being) === hex(inbox._quo.being));
...
test('a standing widened later is re-read from the far door, never remembered', async () => {
  const { rex, inbox, walker } = await world();
  const [handle] = await walker._quo.accept(await rex._quo.grant(rex));
  assert.deepEqual(
    (await handle._quo.handles()).map((one) => hex(one._quo.being)),
    [hex(rex._quo.being)],
  );
  ...
  const widened = await handle._quo.handles();
  assert.equal(widened.length, 2);
  const atInbox = widened.find((one) => hex(one._quo.being) === hex(inbox._quo.being));
...
test("a holder's describe shows what its row names, and never the rest of the estate", async () => {
  const { rex, walker, profile } = await world();
  const [handle] = await rex._quo.accept(await profile._quo.grant(profile));
  const estate = await handle._quo.describe();
  ...
  const sketch = await handle._quo.sketch();
  assert.equal(hex(sketch.being), hex(profile._quo.being));
  assert.equal(await handle._quo.sketch(walker), null);
  // A blueprint by digest, for a class it reaches and for one it does not.
  assert.equal(await handle._quo.blueprint(await digest(parse(PROFILE))), print(parse(PROFILE)));
  assert.equal(await handle._quo.blueprint(await digest(parse(WALKER))), null);
  assert.equal(typeof (await handle._quo.limit()), 'bigint');
});
...
test('a knock is a card turned into a handle at the public being, held as a stranger', async () => {
  ...
  const knocked = await rex._quo.knock(laptop.card());
  assert.equal(hex(knocked._quo.being), hex(laptop.name.pk));

  // The estate a stranger is shown has one room in it: the public being.
  const estate = await knocked._quo.describe();
  ...
  assert.equal(typeof (await knocked._quo.limit()), 'bigint');
  assert.equal(hex((await knocked._quo.sketch(knocked._quo.being)).being), hex(laptop.name.pk));
  assert.equal(await knocked._quo.sketch(walker._quo.being), null);
  assert.equal(await knocked._quo.blueprint(await digest(parse(WALKER))), null);
  // A stranger reaches no being of Bob's, so nothing is accepted from a knock.
  assert.equal(await knocked._quo.handles(), null);
});

test('a same-warden handle answers the same introspection, with nothing judged', async () => {
  const { phone, rex } = await world();
  const { being, handle } = await rex._quo.hold(new Dog('Pup'), { blueprint: DOG, label: 'pup' });
  const reasons = [];
  phone.observe((why) => reasons.push(why));
  ...
  assert.equal(hex((await handle._quo.sketch()).being), hex(being));
  // Outside what a standing at this being would reach: silence, as at a door.
  assert.equal(await handle._quo.sketch(rex), null);
  assert.equal(await handle._quo.blueprint(await digest(parse(DOG))), print(parse(DOG)));
  assert.equal(await handle._quo.blueprint(await digest(parse(WALKER))), null);
  assert.equal(await handle._quo.limit(), phone.limit);
  // Nothing has moved, so `moved` answers absence — the same legal answer to
  // the same legal ask a far door would give.
  assert.equal(await handle._quo.moved(), null);
  assert.deepEqual(
    (await handle._quo.handles()).map((one) => hex(one._quo.being)),
    [hex(being)],
  );
  ...
  rex._quo.release(being);
  assert.equal(await handle._quo.describe(), null);
  assert.equal(await handle._quo.sketch(), null);
  assert.equal(await handle._quo.limit(), null);
  assert.deepEqual(reasons, []);
});

test('a being reaches its warden only through the closure, and never a key', async () => {
  const { rex } = await world();
  const keys = Object.keys(rex._quo);
