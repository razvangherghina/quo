// Writes `quo/vectors/door.json` from the trace: every recipe told against
// both kits, and the file written only when the two recorded the same bytes,
// since a door record is a line two kits agreed on and never one kit's.
//
//   node examples/e2e/trace.js, from the repository's root
...
const NOTE = [
  ...
  'Each record carries the story that puts a kit in its state, as `quo/vectors/HARNESS.md` section 7 says: a stand program started with `stand` beside `--listen` and `--entropy`, the steps of the world the record names, then its own steps, then its arrival.',
