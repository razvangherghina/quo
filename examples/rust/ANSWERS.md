...
25. Only through the `route` request, a map from ward pk to host:port held in memory. The kit writes no `at` in its invitations and does not read one, so it tries no address from an invitation.
