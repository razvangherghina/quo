...
21. Five seconds from each dial. When `at` is tried, each address it dials gets its own five seconds.
...
24. Quo over TCP alone, plain and never inside TLS, as a listener on 127.0.0.1 and as a dialer, one connection for each send. It listens nowhere on the web, so no path, origin or status is its to choose. It stands neither form of Quo over the web, so `listen` of `http` or `ws`, and a route to an address of either, are `bad request`.
25. It learns a ward's address two ways: the `route` request, a map from ward pk to one `tcp://host:port` held in memory, and the `at` of the invitation a send is made on. Once the program holds its listener, every invitation it mints carries that listener's `tcp://127.0.0.1:port` as the one address in `at`; before that, it writes no `at`. A route is dialed alone. With no route, the kit tries the `tcp` addresses of `at` one after another, in the order written, skipping every other scheme, every string that is no `tcp://host:port`, and every element that is not a string. It moves to the next address only when the one before delivered nothing for certain: the connection never opened, the frame never left, or 02 came back. Once a frame has left, the kit waits for that address alone and tries no other, since the ask may have been heard.
